...
- [x] 8.4 Verificação em telas de proporções extremas: celular retrato, celular paisagem, tablet, desktop ultrawide — **verificado no desktop em diversas proporções** (a largura é tratada por classe de janela, coberta por `contentWidth_respeitaBreakpointDeClasse` e `mudancaDeLarguraPreservaPosicaoLogica`); a confirmação em celular/tablet físico permanece recomendada como acompanhamento, não bloqueio
