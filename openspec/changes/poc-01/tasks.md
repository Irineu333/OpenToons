...
- [x] 1.1 Criar módulo Gradle `poc/node` (JVM) com dependência do nabu e verificar que compila
...
- [x] 2.2 Criar módulo `poc/android` mínimo embarcando o nabu e tentar compilar (dex, minSdk, APIs JDK, Netty)
...
- [x] 3.1 Nó `poc/node` conecta ao bootstrap da Amino e participa da DHT como servidor
...
- [x] 5.1 Implementar em `poc/` assinatura e verificação Ed25519 de um manifesto de teste
...
- [x] 7.2 Atualizar referências (roadmap Marco 0 → relatório) e marcar o código de `poc/` como descartável no README do módulo
