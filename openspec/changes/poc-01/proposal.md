...
- Novo módulo descartável `poc/` no repositório com os experimentos (fora dos módulos de produto `shared`/`desktopApp`).
...
- **Código:** novo módulo Gradle `poc/` (JVM) e alvo Android mínimo para E2/E4; nenhum código de produto (`shared`, `desktopApp`) é alterado.
- **Dependências:** nabu (e transitivamente jvm-libp2p/Netty) apenas no módulo `poc/`; biblioteca de crypto Ed25519 para E3.
