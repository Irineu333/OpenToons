...
> Artefato durável do Marco 0. O código em `poc/` é descartável; o que vale é este relatório.
...
|---|---|---|
| nabu (poc/node e poc/android) | v0.8.0 + workarounds da PoC | JitPack (`com.github.Peergos:nabu`) — última tag com build OK lá (0.8.1 e 0.9.x-quic com erro de build). O commit `0b421b9427` (pós-QUIC, build OK no JitPack) foi usado só no diagnóstico de transporte e descartado por regressão no GET_PROVIDERS (ver Desvios) |
...
| AGP (poc/android) | 9.0.1 (Kotlin embutido; série mais nova compatível com Gradle 9.1) | Google Maven |
| Emulador do spike | AVD Galaxy_A54, Android 17 (API 37) | — |
| Dispositivo físico | Moto g(30), Android 12 (API 31) | — |
| JVM (poc/node) | toolchain 21 | — |
...
**Veredito: POSITIVO.** Implementado em `poc/node` (`Manifest.kt` + `ManifestTest.kt`), Ed25519 via BouncyCastle (`Ed25519Signer`) — já no classpath pelo nabu e disponível no Android, ao contrário do provider Ed25519 do JDK. Serialização canônica com length-prefix por campo. 6 testes, todos verdes.
...
- **O commit `0b421b9427` do nabu (pós-QUIC) regrediu o GET_PROVIDERS**: consultas diretas que funcionam no v0.8.0 retornam vazio nele. Por isso `poc/node` voltou ao v0.8.0 (+ workarounds); o commit QUIC fica registrado apenas como fonte do diagnóstico de transporte.
