...
// é escrito 100% contra a interface FullNode do :pocs:poc05:api — zero branch por backend. Os
...
dependencies {
    implementation(project(":pocs:poc05:api"))
    implementation(project(":pocs:poc05:trama"))
    implementation(project(":pocs:poc05:libp2p")) // E2: facade rust estendido com transporte Tor
    implementation(libs.kotlinx.coroutinesCore)
    // vetores/adulteração do TCK reusados pelos drivers de host (mesma obra nas 8 células)
    implementation(testFixtures(project(":pocs:poc05:api")))
...
// o backend libp2p carrega o .dylib de host por JNA (mesmo mecanismo do :pocs:poc05:libp2p:test)
tasks.withType<JavaExec> {
    systemProperty("jna.library.path", project(":pocs:poc05:libp2p").file("nativelib").absolutePath)
}
