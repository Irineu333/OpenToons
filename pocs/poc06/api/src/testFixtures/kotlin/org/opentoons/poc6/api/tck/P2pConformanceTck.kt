package org.opentoons.poc6.api.tck
...
abstract class P2pConformanceTck {
    ...
    private fun standardTopology(
        store: org.opentoons.poc6.api.Blockstore = MemoryBlockstore(),
        publication: org.opentoons.poc6.api.ChapterPublisher.Prepared = TckVectors.prepared(),
    ): Topo {
        val h = harness()
    ...
    private fun fetchAndVerify(client: P2pBackend, provider: Provider): ChapterVerifier.Result {
        ...
        val decoded = org.opentoons.poc6.api.ManifestCodec.decode(manifestBlock)
        val blocks = client.getBlocks(
            provider,
            decoded.manifest.blockCids.map { org.opentoons.poc6.api.ContentId(it) },
        )
    ...
    private fun fetchAndVerifyFrom(client: P2pBackend, provider: Provider): ChapterVerifier.Result {
        val manifestBlock = client.getManifest(provider, TckVectors.OBRA)
        val decoded = org.opentoons.poc6.api.ManifestCodec.decode(manifestBlock)
        val blocks = client.getBlocks(provider, decoded.manifest.blockCids.map { org.opentoons.poc6.api.ContentId(it) })
        return ChapterVerifier(TckVectors.contentKeys.idHex).verify(manifestBlock, blocks.map { it.bytes })
    }
